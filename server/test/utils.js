...
class Spinner {
    constructor(name) {
        this.name = name;
        this.spinner = ora("Testing '" + name + "'").start();
    }

    succeed(description) {
        if (description != null && description != undefined) {
            this.spinner.succeed(`${this.name}: ${description}`);
        } else {
            this.spinner.succeed(`${this.name}`);
        }
    }

    fail(description) {
        if (description != null && description != undefined) {
            this.spinner.fail(`${this.name}: ${description}`);
        } else {
            this.spinner.fail(`${this.name}`);
        }
        process.exit(1);
    }
}
